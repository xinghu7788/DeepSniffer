...
def get_input_sequence (train_inputs,seg_start, seg_end, seg_table):
    length = len(seg_table)
    range_left = seg_table(seg_start)
    if(seg_end == length-1):
        range_right = seg_table[length-1]
    else:
        range_right = int(seg_table[seg_end+1])-1
    print("seg_start, seg_end, input_start, input_end")
    return train_inputs[:,range_left, range_right+1,:]
