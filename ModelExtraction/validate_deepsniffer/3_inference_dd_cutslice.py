...
def next_infer_batch_meta(feats_filename, segs_filename,labels_filename, start_marker, end_marker):
    train_inputs = np.load(feats_filename)  # 3-D
    seg_table = np.load(segs_filename)
    train_targets = np.load(labels_filename)
    length_seg_table = len(seg_table)
    print('length',length_seg_table)
    op_list = []
    #print('input :??????????????????????', train_inputs)
    if start_marker >= length_seg_table:
        return None, None, None, None;
    if end_marker >= length_seg_table:
        end_marker = length_seg_table-1
    
    from roi_selection_incept_dd import deterministic_select_seg           
    train_inputs, train_targets_sparse, original = deterministic_select_seg(train_inputs, train_targets, seg_table, start_marker, end_marker )  # 0 for scheduler
...
def run_ctc(log_dir, args):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for index_val in range(0,11):
            index_val = index_val + 1
            sample_file = args.sample_file
            label_file = args.label_file
            seg_file = args.seg_file
            start_marker = (index_val-1)*slice_size
            end_marker = index_val*slice_size
            
            val_inputs, val_targets, val_seq_len, val_original  = next_infer_batch_meta(sample_file, seg_file, label_file, start_marker, end_marker)
            #print(val_inputs)
