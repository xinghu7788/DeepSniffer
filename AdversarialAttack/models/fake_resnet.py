...
class ResNet(nn.Module):
    def __init__(self, num_classes=1000):
        ...
        self.layer4 = self._make_layer(channels[3], stride=strides[3], if_relu=True)
...
def fake_1(pretrained):
    global w_size
    w_size = 3
    model = ResNet()
    if pretrained != 'null':
        m = torch.load(pretrained)
        #print(m['layer1.0.conv1.weight'].shape)
        model.load_state_dict(torch.load(pretrained))

    return model

def fake_2(pretrained=None):
    global downsample_kernel, w_size, channels, strides
