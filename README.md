...
1) To reproduce the results of Table6, run python DeepSniffer/AdversarialAttack/AdversarialAttack.py. 
    <p>python DeepSniffer/AdversarialAttack/AdversarialAttack.py --iter_number=20 </p> 
    where iter_number is how many image examples (100 as the unit number in every iteration) that you would like to run
2) To reproduce the results of Figure 10, run python DeepSniffer/AdversarialAttack/auto_attack_labels_random.py 750 245. where 750 and 245 is the source label and the 245 is the target label. You can change as you need.
